...
export class RepresentantService {
	...
	async create(
		createRepresentantDto: Representant,
	): Promise<Representant | string> {
		const existingRepresentant = await this.findRepresentant(
			createRepresentantDto.name,
		);
		if (existingRepresentant)
			return "Esse representante já está cadastrado";
		const user = await this.prisma.representant.create({
